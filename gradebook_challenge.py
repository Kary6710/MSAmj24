#list_of lists
def main():
    list_of_lists= [[61, 74, 69, 62, 72, 66, 73, 65, 60, 63, 69, 63, 62, 61, 64],
    ...
    #make a loop to get index for nembers and loop back
    print(f"\n")
    for column in range(len(list_of_lists[0])):
        print(f"Asighnment: {column + 1}")
        asighnment_total = 0
        for row in range (len(list_of_lists)):
            asighnment_total += list_of_lists[row][column]
        asighnment_average = asighnment_total/ (len(list_of_lists))
        print(f"{asighnment_average:.2f}%")
    print(f"\n")
    for row in range (len(list_of_lists)):
        print(f"Student Average: {row + 1}")
        student_total = 0 
        for column in range (len(list_of_lists[row])):
            student_total += list_of_lists[row][column]
        student_average = student_total/(len(list_of_lists[row])) 
...
for row in range(len(gradebook)):
    ...
    print(f"Studnet{row + 1}: {average:.2f}")
