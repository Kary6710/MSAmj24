...
#function to load vehivle data from file
#input: path to the file
#Output: list of automobiles

def load_vehicles(file_name):
    #create an empty list to store automobilee data
    list_of_automobiles = []
    #open the file
    auto_file = open(file_name,"r") 
    #read each line of the file in a four loop

    #ignor the header data row in the file
    #auto_file.readline()
    next(auto_file)
    ...
    for line_of_data in auto_file:
        ...
        #check that vehicle data contains 6 items
        ...
        try:
            ...
            if engine_size[3].lower() == "electric":
                engine_size = 0
            else:
        ...
        #close file a return the list of automobiles
    auto_file.close()
...
def main():
    ...
    automobile_list = load_vehicles("vehicle_data (1).csv")
